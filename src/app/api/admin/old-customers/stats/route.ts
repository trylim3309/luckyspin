  ...
  try {
    ...
    if (isAgent) {
      const [
        ...
        prisma.oldCustomer.count({ where: buildWhere({ ...depositCondition, createdAt: { gte: todayStart } }) }),
        ...
        prisma.oldCustomer.count({ where: buildWhere({ createdAt: { gte: todayStart } }) }),
        prisma.oldCustomer.count({ where: buildWhere({ createdAt: { gte: todayStart }, callStatus: "NOT_CONTACTED" }) }),
        prisma.oldCustomer.count({ where: buildWhere({ createdAt: { gte: todayStart }, result: "NOT_DEPOSIT" }) }),
        prisma.oldCustomer.count({ where: buildWhere({ createdAt: { gte: todayStart }, result: "DEPOSIT" }) }),
    ...
    if (filterAgentId && filterAgentId !== "all") {
      ...
      const [todayCount, weekCount, monthCount, lastMonthCount,
        ...
        prisma.oldCustomer.count({ where: buildWhere({ agentId: filterAgentId, ...depositCondition, createdAt: { gte: todayStart } }) }),
        ...
        prisma.oldCustomer.count({ where: buildWhere({ agentId: filterAgentId, createdAt: { gte: todayStart } }) }),
        prisma.oldCustomer.count({ where: buildWhere({ agentId: filterAgentId, createdAt: { gte: todayStart }, callStatus: "NOT_CONTACTED" }) }),
        prisma.oldCustomer.count({ where: buildWhere({ agentId: filterAgentId, createdAt: { gte: todayStart }, result: "NOT_DEPOSIT" }) }),
        prisma.oldCustomer.count({ where: buildWhere({ agentId: filterAgentId, createdAt: { gte: todayStart }, result: "DEPOSIT" }) }),
    ...
    const [todayStats, weekStats, monthStats, lastMonthStats,
      ...
      prisma.oldCustomer.groupBy({ by: ["agentId"], where: buildWhere({ ...depositCondition, agentId: { in: agentIds }, createdAt: { gte: todayStart } }), _count: true }),
      ...
      prisma.oldCustomer.groupBy({ by: ["agentId"], where: buildWhere({ agentId: { in: agentIds }, createdAt: { gte: todayStart } }), _count: true }),
      prisma.oldCustomer.groupBy({ by: ["agentId"], where: buildWhere({ agentId: { in: agentIds }, createdAt: { gte: todayStart }, callStatus: "NOT_CONTACTED" }), _count: true }),
      prisma.oldCustomer.groupBy({ by: ["agentId"], where: buildWhere({ agentId: { in: agentIds }, createdAt: { gte: todayStart }, result: "NOT_DEPOSIT" }), _count: true }),
      prisma.oldCustomer.groupBy({ by: ["agentId"], where: buildWhere({ agentId: { in: agentIds }, createdAt: { gte: todayStart }, result: "DEPOSIT" }), _count: true }),
